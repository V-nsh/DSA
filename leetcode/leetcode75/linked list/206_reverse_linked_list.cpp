...
public:
    ListNode* reverseList(ListNode* head) {
        ...
        // LL is more of visualisation, the head will reverse its next and point to the left, ie nullpts
        // head's previous next will be it's previous, and it's next will point to the head
        // eg for: head -> next -> third -> ..
        // will become: null <- head <- next <- third ..
        // thus we will keep a track of next node before changing it's next to the previous one. 
        while(curr != nullptr) {
            nextNode = curr->next;
            curr->next = prev;
            prev = curr;
